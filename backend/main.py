...
def export_csv(scenario_id: int, db: Session=Depends(get_db)):
    ...
    buf = io.StringIO()
    w = csv.writer(buf)
    w.writerow(["timestamp","temperature_c","humidity_pct","soil_moisture_pct","light_lux","vpd_kpa","eto_mm_h",
                "overall_stress","stress_level","dominant_stress"])
    for r in readings:
        ro = _reading_out(r,s)
...
def get_summary(scenario_id:int,db:Session=Depends(get_db)):
    s = _get_or_404(scenario_id, db)
    readings = db.query(Reading).filter(Reading.scenario_id == s.id).order_by(Reading.timestamp).all()
    if not readings: return ScenarioSummary(
        scenario=_scenario_out(s, db),total_readings=0,avg_overall_stress=0,max_overall_stress=0,time_in_stress=
        {"optimal":100,"mild":0.0,"moderate":0.0,"severe":0.0},dominant_stresses={},avg_vpd=0.0, avg_eto=0.0,
        avg_temp=0.0,avg_humidity=0.0,)
...
def compute_adhoc(body:ComputeRequest):
    now = datetime.utcnow()
    sr = compute_stress_indices(
        T=body.temperature_c, RH=body.humidity_pct,lux=body.light_lux,wind_speed_ms=body.wind_speed_ms,
        soil_moisture_pct=body.soil_moisture_pct, elevation_m=body.elevation_m,latitude_deg=body.latitude_deg,day_of_year=now.timetuple().tm_yday,hour=now.hour,
    )
